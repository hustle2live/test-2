(function() {
    ...
    function addDetectionHooks(root, scopeName) {
        ...
        for (let e = 0; e < triggers.length; e++) {
            ...
            triggers[e].addEventListener(config.detectionEventType, clickFunc, {
                once: true,
                capture: true
            });
        }
    ...
    function clearDetectionHooks(scopeName) {
        ...
        if (scopeName === "all") {
            for (const [sname, triggers] of Object.entries(scopedTriggers)) {
                if (triggers) {
                    for (let e = 0; e < triggers.length; e++) triggers[e].removeEventListener(config.detectionEventType, scopedEventListeners[sname][e], {
                        capture: true
                    });
                }
            }
    ...
    function collectElements(root, scopeName, cssSelectors) {
        ...
        for (let n = 0; n < t.length; n++) {
            ...
            if (canAccessIframe(h, iframe)) {
                iframe.__upixel_detection = {
                    root: root,
                    scopeName: scopeName + "/iframe"
                };
...
function _TTDUniversalPixelApi(optionalTopLevelUrl) {
    ...
    const advertisersOptedInToRealtime = ["ru6qq3d", "ufqc9us", "ugj89je", "foh1kju", "1znfrjo", "xjagv7s"];
    ...
    async function initializePixel(adv, tag_ids, base_src, dyn_params, uid_config, route_realtime_api) {
        ...
        function isUidConfigCstgValid(uid_config) {
        ...
        async function fireOrDetect(identity_getter, uid_config) {
            try {
                ...
                if (!identity && isUidConfigCstgValid(uid_config)) {
        ...
        async function fireRealtimeConversionEventFallback(paramMap) {
            let iFrameId = "universal_pixel_" + tag_ids.join("_");
            let title = "TTD Universal Pixel Realtime";
            paramMap["eds"] = "UpSdkGet";
            paramMap["edsv"] = getVersion();
            paramMap["upid"] = tag_ids.join(",");
            const {
                pixel_ids,
                ...paramMapNoPixelIds
            } = paramMap;
            const params = new URLSearchParams(paramMapNoPixelIds);
            const rt_get_url = `https://${getHostname(base_src)}/track/rt`;
            const rt_get_url_with_params = `${rt_get_url}?${params.toString()}`;
            await createIFrameInternal(rt_get_url_with_params, iFrameId, title);
        }

        async function fireRealtimeConversionEvent(paramMap, advertising_token) {
            paramMap["pixel_ids"] = tag_ids;
            ...
            return new Promise((resolve, reject) => {
                ...
                xmlHttp.onerror = async () => {
                    reject(xmlHttp.statusText);
                    await fireRealtimeConversionEventFallback(paramMap);
                }
