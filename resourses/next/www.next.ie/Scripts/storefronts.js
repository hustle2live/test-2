...
function onCreditTrayLoaded(theIframe) {
    ...
    $innerDoc.find(".cm_bootstrap").css({
        'margin-top': '0'
    });
    $innerDoc.find(".Background").css({
        'padding': '0'
    });
    $innerDoc.find(".cm-teaser-overlay__text").css({
        'padding': '10px 15px'
    });
    $innerDoc.find(".ContentArea").css({
        'min-width': 'auto',
        'width': '941px',
        'padding-bottom': '0'
    });
    $innerDoc.find(".StorefrontContent").css({
        'width': '941px'
    });
    $innerDoc.find(".Storefront").css({
        'padding-bottom': '0'
    });
    $('.iframeLoader').switchClass('showLoader', 'hideLoader');
    $innerDoc.find(".Storefront #cm-placement-placement2 span a[name=\"terms\"]").css({
        'pointer-events': 'none'
    });
    $innerDoc.find(".cm-richtext .hp-btn").css({
        'width': 'auto'
    });
