...
function PromoPrice(props) {
    ...
    const economyCalcul = (initialPrice, discountPrice) => {
        ...
        return economy.toLocaleString('fr-FR', {
    ...
    return ( <
            ...
            : data.refPrice > data.price && amountDiscount ? <span>{formatedPrice(data.refPrice)} &# 8364; < /span> :
                    ...
                    /div> <
                    div className = {
                        `helper-blocPrice relative OpenSans-eb fw8 ${stylePrice}`
                    } >
                    <
                    p className = "f54 dib" > {
                        Math.trunc(data.price)
                    } < sup className = "f22 price-euro-full" > € < /sup></p > {
                        getFraction(data.price) !== ',00' ? < p className = "price-decimal-full_large-device dib f22" > {
                        ...
                        /div> { 
                            data.originalPrice && data.originalPrice > data.price && amountDiscount ?
                                ...
                                <
                                p > Soit < span className = "b" > {
                                    economyCalcul(data.originalPrice, data.price)
                                } & #8364;</span> d'économie</p>
        </div>
      </div>
        ...
        <div className= "promo-price-economy dib center f14 black bg-lightgray ph3 pv2 mt2" >
                                ...
                                <
                                p > Soit < span className = "b" > {
                                    economyCalcul(data.refPrice, data.price)
                                } & #8364;</span> d'économie</p>
          </div>
        </div>
        :
        <div className= "fixed-height-31" > < /div>
            ...
            : data.refPrice > data.price && amountDiscount ? <span>{formatedPrice(data.refPrice)} &# 8364; < /span> :
                            ...
                            /p> {
                                amountDiscount ? < span className = "white flex items-center b f10 flux-promo-pct-discount" > -{
                                        amountDiscount
                                    } % < /span>  :
                                    < span className = "fixed-height-15" > < /span> 
                            } <
                            /div> <
                            div className = {
                                `relative OpenSans-eb fw8 ${stylePrice}`
                            } >
                            <
                            p className = "f34 dib" > {
                                Math.trunc(data.price)
                            } < sup className = "f16 price-euro-full_small-device" > € < /sup></p > {
                                getFraction(data.price) !== ',00' ? < p className = "price-decimal-full_small-device dib f16" > {
                                    getFraction(data.price)
                                } < /p> : '' } <
                                /div> {
                                    data.originalPrice && data.originalPrice > data.price && amountDiscount ?
