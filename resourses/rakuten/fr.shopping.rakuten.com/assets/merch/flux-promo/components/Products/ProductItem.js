...
const ProductItem = ({
    ...
    const isApp = user.isFluxPromoApp || false;
    ...
    const advertUrl = item.selected_advert.url;
    const category = item.product.category;
    const subCategory = item.product.subcategory;
    const advertType = item.selected_advert.advert_type || '';
    const qualityType = item.selected_advert.quality || '';
    const isRefurbished = item.selected_advert.refurbished ? item.selected_advert.refurbished : false;
    ...
    const quantity = item.selected_advert.quantity || 0;
    const isFrenchVendor = item.selected_advert.isFrenchVendor && (sellerName !== 'cat-pm2') ? true : false;
    ...
    if (type === 'advert') {
        itemUrl = advertUrl + xtatc;
    } else if (type === 'product' && productUrl.includes('offer/')) {
        itemUrl = productUrl + '?bbaid=' + advertId + '&filter=20' + xtatc;
    } else if (type === 'product') {
        itemUrl = productUrl + '&bbaid=' + advertId + '&filter=20' + xtatc;
    } else if (productUrl.includes('offer/')) {
